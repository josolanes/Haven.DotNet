...
public static class ServiceExtensions
{
	extension(IServiceCollection services)
	{
		...
		public IServiceCollection AddHavenDotNet<T>(string baseUrl, string token) where T : class, IHavenDotNetHandler
		{
			...
			services.AddTransient<IHavenService>(provider => new HavenService(provider.GetRequiredService<IHttpClientFactory>(), token));
